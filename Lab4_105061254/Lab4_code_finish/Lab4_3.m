...
h_plot = plot(nan,nan);
hold on
h_plot2 = plot(nan,nan,'rv','MarkerFaceColor','r');
hold off 
tic
...
for i= 1:5000
    ...
    % threshold
    [~,locsR] = findpeaks(y5,'minpeakheight',100,'minpeakdistance',100);
    locs = (locsR-1)/fs;

    if i>1 && rem(i,40)==0
    plot(time,y2,'b',locs,y2(locsR),'ro');
    axis([ min(time) max(time) 0 800 ]);
    xlabel('Time(s)');
    ylabel('Quantization value');
    
    if i>700
        rate=60/(locs(3)-locs(2));
    end
    str=['Heart Rate: ',num2str(rate),' per minute'];    
    title(str);
    drawnow;
    end
end
